...
class RestorDiffModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(RestorDiffModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        change
        '''
        self.output = self.net_g(self.gt, self.lq)
        ''''''
        l_total = 0
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                change
                '''
                self.output = self.net_g_ema.sample(self.lq)
                ''''''
        else:
            self.net_g.eval()
            with torch.no_grad():
                ...
                change
                '''
                self.output = self.net_g.sample(self.lq)
                ''''''
            self.net_g.train()
