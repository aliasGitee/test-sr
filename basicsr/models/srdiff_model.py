...
class SRDiffModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        change
        '''
        self.output = self.net_g(self.gt, self.lq)[0]['q']
        ''''''
        l_total = 0
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                change
                '''
                self.output = self.net_g_ema.sample(self.lq)[0]
                ''''''
        else:
            self.net_g.eval()
            with torch.no_grad():
                ...
                change
                '''
                self.output = self.net_g.sample(self.lq)[0]
                ''''''
            self.net_g.train()
