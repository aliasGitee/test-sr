...
# x -> linear -> channel_split -> x,z -> (x: -> dwconv -> ss2d -> layernorm) ->  x * F.silu(z) -> linear -> output
...
class myVSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = mySS2D(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
...
# VSSBlock -> downsample:PatchMerge,c*2,hw/2
...
# VSSBlock -> upsample:PatchExpand,c//2,hw*2
...
if __name__ == "__main__":
    ...
    print(out.shape)
    out = out.view(b, 1, -1, d_in)
    print(out.shape)
    y = torch.transpose(out, dim0=1, dim1=2).contiguous().view(b, 48, 48, -1)
    print(y.shape)
