...
from basicsr.archs.emt.utils import MeanShift, Conv2d1x1, Conv2d3x3, ShiftConv2d1x1, PixelMixer, \
    SWSA, TransformerGroup, Upsampler, Swish
from basicsr.archs.efficientvit.fix.ops_fix import LiteMLAFix
...
class MixedTransformerBlock(TransformerGroup):
    def __init__(self, dim: int, num_layer: int, num_heads: int, num_GTLs: int,
        ...
        token_mixer_list = [TokenMixer(dim) if _ > (num_GTLs - 1) else LiteMLAFix(in_channels=dim,
                                                                                  out_channels=dim,
                                                                                  dim=dim//4,norm=(None,"ln2d"))
...
class EMT(nn.Module):
    ...
    def __init__(self, upscale: int, num_in_ch: int, num_out_ch: int, task: str, dim: int, n_blocks: int, n_layers: int,
        ...
        self.sub_mean = MeanShift(255, sign=-1, data_type='DIV2K')
        self.add_mean = MeanShift(255, sign=1, data_type='DIV2K')
