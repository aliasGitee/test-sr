...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample_ddim(self, img_lr,img_lr_up, shape,sampling_timesteps=50,clip_denoised=True):
        device = self.betas.device
        b = shape[0]
        if not hparams['res']:
            t = torch.full((b,), self.num_timesteps - 1, device=device, dtype=torch.long)
        ...
        it = tqdm(time_pairs, desc='sampling loop time step', total=len(time_pairs))
        for time, time_next in it:
            time = torch.full((b,), time, device=device, dtype=torch.long)
            time_next = torch.full((b,), time_next, device=device, dtype=torch.long)
            noise_pred = self.denoise_fn(img, time, cond=cond, img_lr_up=img_lr_up)
            x_recon = self.predict_start_from_noise(img, t=time, noise=noise_pred)
            if clip_denoised:
                x_recon.clamp_(-1.0, 1.0)
            if time_next[0] < 0:
                img = x_recon
            ...
            img = x_recon * alpha_next.sqrt() + c * noise_pred + sigma * noise
        img = self.res2img(img, img_lr_up)
        return img, rrdb_out
