...
import math
import basicsr.archs.bsrn.Upsamplers as Upsamplers
...
class BSRN2(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=60, num_block=8, num_out_ch=3, upscale=4,
        ...
        self.conv = EConvMixer
        self.fea_conv = self.conv(num_in_ch * 4, num_feat, kernel_size=3)

        self.B1 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B2 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B3 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B4 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B5 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B6 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B7 = ESDB(in_channels=num_feat, conv=self.conv)
        self.B8 = ESDB(in_channels=num_feat, conv=self.conv)
